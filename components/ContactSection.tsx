...
import { MOCK_SITE_SETTINGS } from '../utils/mockData';
      ...
      <GlassCard className="flex-1 relative overflow-hidden group !p-8 flex flex-col justify-between" hoverEffect={true}>
        ...
        <div className="space-y-8 relative z-10">
          <div>
            ...
            <p className="text-slate-400 pl-4 border-l border-white/10 ml-1 py-1 whitespace-pre-line">
              {MOCK_SITE_SETTINGS.galleryAddress}
            </p>
          </div>
...
export const ContactForm: React.FC = () => {
  ...
  const inputClasses = "w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder-slate-600 focus:outline-none focus:border-white focus:ring-1 focus:ring-white transition-all duration-300 hover:bg-white/[0.07]";
  const labelClasses = "block text-xs font-bold uppercase tracking-[0.2em] text-slate-400 mb-2";

  return (
    <GlassCard className="!p-8 md:!p-10 backdrop-blur-xl bg-black/40 relative overflow-hidden">
      
      {success && (
        <div className="absolute inset-0 z-20 bg-black/95 flex items-center justify-center flex-col animate-in fade-in">
                ...
                <div className="bg-black/20 p-1 rounded-lg border border-white/5 flex relative">
                  <button
                    ...
                    className={`flex-1 relative z-10 py-2 rounded-md text-xs md:text-sm font-medium transition-all duration-300 ${
                        method === 'Walk-in' ? 'text-black' : 'text-slate-500 hover:text-slate-300'
                    }`}
                  >
                    Showroom Visit
                  </button>
                  <button
                    ...
                    className={`flex-1 relative z-10 py-2 rounded-md text-xs md:text-sm font-medium transition-all duration-300 ${
                        method === 'Pre-Order' ? 'text-black' : 'text-slate-500 hover:text-slate-300'
                    }`}
                  >
                    Pre-Order
                  </button>
                  <div 
                    className={`absolute top-1 bottom-1 w-[calc(50%-4px)] bg-white rounded shadow-lg transition-transform duration-300 ease-out ${
