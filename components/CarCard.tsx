    ...
    <GlassCard className="group relative overflow-hidden !p-0 h-full flex flex-col" hoverEffect={true}>
      ...
      <div className="relative aspect-[16/10] overflow-hidden bg-white/5">
        <div className={`absolute top-4 left-4 z-20 px-3 py-1 rounded text-xs font-bold tracking-widest uppercase ${
          isPreOrder ? 'bg-white text-black' : 'bg-white/10 backdrop-blur-md text-white border border-white/20'
        }`}>
          {car.status}
        </div>
      ...
      {/* Content Section */}
      <div className="p-6 flex flex-col flex-grow relative z-20 bg-gradient-to-b from-transparent to-black/50">
        
        <div className="mb-4">
          <p className="text-white text-xs font-bold tracking-[0.2em] uppercase mb-1">{car.brand}</p>
          <h3 className="font-serif text-2xl text-white leading-tight group-hover:text-slate-300 transition-colors">
            {car.model}
          </h3>
        </div>
        ...
        <div className="mt-auto flex items-end justify-between">
          ...
          <button className="w-10 h-10 rounded-full border border-white/50 text-white flex items-center justify-center group-hover:bg-white group-hover:text-black transition-all duration-300">
